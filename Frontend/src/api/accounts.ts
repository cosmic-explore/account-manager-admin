...
import { API_ROOT, buildGetRequest, buildUpdateRequest, handleServerError } from "./util"

export const requestAccounts = async () => {
    const response = await fetch(`${API_ROOT}/accounts`, buildGetRequest())
    handleServerError(response, 'Error retrieving accounts')
    return response.json()
}

export const requestAccountResources = async (accountId: string) => {
    const response = await fetch(`${API_ROOT}/accounts/${accountId}/resources`, buildGetRequest())
...
export const requestCreateAccount = async (name: string, status: string) => {
    ...
    const response = await fetch(`${API_ROOT}/accounts`, buildUpdateRequest('POST', requestBody))
...
export const requestUpdateAccount = async (accountId: string, updateData: AccountUpdateData) => {
    ...
    const response = await fetch(`${API_ROOT}/accounts/${accountId}`, buildUpdateRequest('PATCH', requestBody))
