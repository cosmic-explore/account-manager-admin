...
import { API_ROOT, buildUpdateRequest, handleServerError } from './util'

export const requestCreateResource = async (createData: ResourceCreateData) => {
    const requestBody = JSON.stringify(createData)
    const response = await fetch(`${API_ROOT}/resources`, buildUpdateRequest('POST', requestBody))
